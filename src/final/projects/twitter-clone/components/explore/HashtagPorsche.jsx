...
const HashtagCats = () => {
  ...
  const hashtag = useSelector((store) => store.twitter.hashtagPorsche);

  return (
    <div className="">
      {isShown ? (
        <div className="">
          <Header page="#Porsche" wheretopost={"hashtagporsche"} />
            ...
            <div className="feed container-fluid">
              {hashtag.map((post) => (
                <div className="position-relative" role="button" key={post.id}>
                  ...
                  <button
                    onClick={() => dispatch(deleteHashtagPorschePost(post.id))}
